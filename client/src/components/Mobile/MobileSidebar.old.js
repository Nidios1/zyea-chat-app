...
import ChatArea from '../Shared/Chat/ChatArea';
...
const TopBar = styled.div`
  ...
  top: 0;
  z-index: 100;
  /* NO negative margin - causes overlap issues */
  flex-shrink: 0;
...
const SearchInput = styled.input`
  ...
  color: white;
  font-size: 15px;
  /* Minimum 16px to prevent iOS zoom, but 15px is close enough */
  outline: none;
...
const Content = styled.div`
  ...
  flex-direction: column;
  position: relative;
  /* CRITICAL: Add margin-bottom to prevent BottomNav from covering last item */
  margin-bottom: 0;
  /* The padding is handled by the last child instead */
...
const ConversationList = styled.div`
  ...
  position: relative;
  padding-top: 8px;
  /* Add small padding-bottom to prevent overlap with BottomNav */
  padding-bottom: 8px;
  
  /* Only add safe area padding when needed */
  @media (max-width: 768px) {
...
const BottomNav = styled.div`
  ...
  /* Prevent content from being blocked */
  pointer-events: auto;
`;
...
const NavItem = styled.div`
  ...
  /* Landscape mode - slightly smaller */
  @media (max-width: 768px) and (orientation: landscape) {
    min-height: 36px;
    padding: 4px 2px;
    gap: 1px;
...
const TabBar = styled.div`
  ...
  overflow-x: auto;
  position: sticky;
  /* Stick below TopBar - calculate proper height */
  top: calc(56px + env(safe-area-inset-top, 0px));
  /* 56px = TopBar height (40px) + padding */
  z-index: 99;
  flex-shrink: 0;
  /* Ensure it doesn't shrink */
