...
const ChatArea = ({ conversation, currentUser, socket, onMessageSent, onSidebarReload, isMobile = false, onBackToSidebar, currentView = 'conversation' }) => {
  ...
  const {
    ...
    resetSwipe,
    getNavigationTarget
  } = useSwipeNavigation(currentView);
  
  // Debug log
  console.log('ChatArea isMobile:', isMobile);
  ...
  // Enhanced touch handlers with smart navigation
  ...
  const handleTouchEndEnhanced = (e) => {
    if (!isMobile) return;
    handleTouchEnd(e, handleSmartNavigation);
  };
  ...
  const handleSendImage = async (file) => {
    ...
    try {
      ...
      // Upload image
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://192.168.0.101:5000/api'}/chat/upload-image`, {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    >
      <SmartNavigationIndicator 
        visible={swipeOffset > 20 && isMobile}
